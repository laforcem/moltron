...
- **yt-dlp**: standalone binary from the **nightly** GitHub release channel (not stable, not apt/pip) — YouTube breaks extractors often enough that yt-dlp's own maintainers recommend nightly for regular users, and stable's ~monthly cadence lags too far behind. Installed to `/home/moltron/.local/bin/yt-dlp`, moltron-owned (not root), with a matching `systemd --user` timer (`yt-dlp-update.timer`, daily) running `yt-dlp -U` — the binary's own self-updater, which only works on this standalone form (pip/apt installs can't self-update). moltron owns it so the daily update runs unprivileged like everything else moltron runs, instead of needing a root-level periodic job just to keep one CLI tool current.
- **ffmpeg**: plain `apt install ffmpeg` — unlike yt-dlp it isn't in an active arms race with a platform, and Debian 13's package is a full build (`libx264` etc., not the old crippled Ubuntu/Debian split). Riding apt means it's patched automatically by `homelab`'s unattended-upgrades instead of this repo tracking a second binary source.
