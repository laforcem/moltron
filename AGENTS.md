...
Full details in README — don't duplicate here, but the short version: `moltron-bot` is a plain machine-user account (not a GitHub App — see issue #2) with push access to both `laforcem/moltron` and `laforcem/homelab`. As of 2026-09-18, authenticated via `gh`'s own native OAuth login (`gh auth git-credential` as the git credential helper for both `github.com` and `gist.github.com`) — **not** a classic PAT/`GH_TOKEN` anymore. That's a reversal of the README's older stated reasoning ("not `gh auth login`: it hard-requires classic-token scopes the bot doesn't need or have") — the native login already active on the box has those exact scopes anyway, and the bot has its own account, so the extra scopes aren't a real concern. The switch happened after the previous `GH_TOKEN` PAT was accidentally exposed and revoked (an agent's own shell-quoting mistake while testing the CLI-credential egress proxy below, not a leak of this repo's code) — rather than rotate it back into a plaintext env file (`~/.profile`, `~/.config/environment.d/`), git was pointed at the credential OpenClaw's own agent was never trusted to see in the first place. Default identity on the `moltron` Unix user, but override per-commit based on autonomy level:
...
- Known limitation, accepted as-is: for the locally-run-program shape, "credential resolved but the real service rejected it" only reliably surfaces as a distinct plugin-level message when the spawned program actually exits (confirmed for Actual's network case via a real HTTP 401). Confirmed live against Tandoor with a genuinely wrong credential: `tandoor-recipes-mcp` doesn't crash on a real 403 from Tandoor's API — it returns the error as an ordinary tool result instead, which the agent reports correctly and specifically on its own ("returned 403 Forbidden..."), just not also through the plugin's own health-status channel. The real, useful information does reach whoever's using the integration either way; fixing this fully would mean parsing MCP JSON-RPC traffic on the wire, which wasn't judged worth the added complexity.
- Live on `valet`: `actual` (network), `tandoor` and `hardcover` (locally-run-program). `hardcover`'s credential was previously plaintext in `mcp.servers.hardcover.env.HARDCOVER_API_TOKEN` despite an unused protected `HARDCOVER_API_TOKEN` already sitting in the store — migrated the same way as Tandoor.
- Not every credential fits this plugin, even when it looks similar: `GH_TOKEN` (git's own credential, see GitHub identity above) turned out not to need protecting here at all — `gh`'s own native OAuth login already covered it once git was pointed at `gh auth git-credential` instead. A `~/.gitconfig` custom credential helper reading `$GH_TOKEN` directly, independent of `gh`'s own auth, is exactly the kind of thing worth checking for before assuming a static credential needs migrating rather than removing outright.
...
`~/.config/rclone/rclone.conf` currently stores the Dropbox `client_secret`/OAuth `token` as plain, unencrypted text — confirmed directly (the file has a plain readable `[dropbox]` header, not rclone's encrypted-config marker), not assumed. rclone does support real config-file encryption, just not turned on here; doing so properly needs the same kind of credential-supervision the `moltron-mcp-guard` plugin does for locally-run MCP servers, applied to a standing background process instead of a per-request one — tracked as its own follow-up, not started: issue #40.
